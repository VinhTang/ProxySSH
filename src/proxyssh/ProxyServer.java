...
import java.io.*;
import java.net.*;
import ProxySSH.Server.*;

/**
 * This class uses the Server class to provide a multi-threaded server framework
 * for a relatively simple proxy service. The main() method starts up the
 * server. The nested Proxy class implements the Server.Service interface and
 * provides the proxy service.
 *
 */
...
//          System.out.println(temp+ ": " +args[temp]);
//          temp++;
//      }
        try {
      ...
      // Loop through the arguments parsing (host, remoteport, localport)
            ...
            while (i < args.length) {
                ...
                System.out.println("ra toi day");
                s.addService(new Proxy(host, remoteport), localport);
                System.out.println("pass " + host + " - " + remoteport + " - " + localport);
     ...
     * forth between client and server. For symmetry, this class implements two
     * very similar threads as anonymous classes. One thread copies bytes from
     * client to server, and the other copies them from server to client. The
     * thread that invoke the serve() method creates and starts these threads,
     * then just sits and waits for them to exit.
   *
     */
